...
struct cudaGraphicsResource
    *cuda_vbo_color_resource;      // handles OpenGL-CUDA exchange
...
extern "C" void feelTheAttraction(cData *v, int dimx, int dimy, float3 position,
...
void initParticlesFromTexture(cData *p, int dx, int dy,
...
void TexFunc(void) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, 3, 640, 480, 0, GL_RGBA, GL_UNSIGNED_BYTE,
               ubImage);
  glBegin(GL_POLYGON);
...
// TexFunc();
  ...
  if (fpsCount == fpsLimit) {
    ...
    sprintf(fps, "Cuda/GL Stable Fluids (%d x %d): %3.1f fps", DIMX, DIMY,
            ifps);
...
void keyboard(unsigned char key, int x, int y) {
  ...
  case 'r':
    ...
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(GLfloat) * 3 * DS,
                    particuleColors, GL_DYNAMIC_DRAW_ARB);
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);

    cudaGraphicsGLRegisterBuffer(&cuda_vbo_resource, vbo,
                                 cudaGraphicsMapFlagsNone);
    cudaGraphicsGLRegisterBuffer(&cuda_vbo_color_resource, vbo_color,
                                 cudaGraphicsMapFlagsNone);
...
void timerEvent(int value) {
  ...
  kinectPosition.x = (positions[NUI_SKELETON_POSITION_HEAD].x +
                      positions[NUI_SKELETON_POSITION_HAND_LEFT].x +
                      positions[NUI_SKELETON_POSITION_HAND_RIGHT].x) /
                     3.f;
  kinectPosition.y = -(positions[NUI_SKELETON_POSITION_HEAD].y +
  ...
  feelTheAttraction(dvfield, DIMX, DIMY, kinectPosition, gTimer,
                    gParam /*+20*cos(gTimer)*/);
...
int main(int argc, char **argv) {
  ...
  cutilSafeCall(cudaGraphicsGLRegisterBuffer(
      &cuda_vbo_color_resource, vbo_color, cudaGraphicsMapFlagsNone));
  cutilCheckMsg("cudaGraphicsGLRegisterBuffer failed");
